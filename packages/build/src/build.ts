...
const debugViewDist = join(root, '.tmp', 'dist-chat-storage-worker')
const networkWorkerDist = join(root, '.tmp', 'dist-chat-storage-worker')
...
const debugViewPackageJson = await readJson(join(root, 'packages', 'chat-storage-worker', 'package.json'))
...
debugViewPackageJson.main = 'dist/chatStorageWorkerMain.js'
